...
class tsOutput
{
	...
	public function get_selected_ts_output($selected_id) {
		...
		$this->main->utils()->put_process_log(__METHOD__, __LINE__, '■ get_selected_ts_output start');
		...
		$this->main->utils()->put_process_log(__METHOD__, __LINE__, '■ get_selected_ts_output end');
	...
	public function insert_ts_output($dataArray) {
		...
		$this->main->utils()->put_process_log(__METHOD__, __LINE__, '■ insert_ts_output start');
		...
		$this->main->utils()->put_process_log(__METHOD__, __LINE__, '■ insert_ts_output end');
	...
	public function update_ts_output($output_id, $dataArray) {
		...
		$this->main->utils()->put_process_log(__METHOD__, __LINE__, '■ update_ts_output start');
		...
		$this->main->utils()->put_process_log(__METHOD__, __LINE__, '■ update_ts_output end');
	}
	...
	private function convert_ts_output_entity($array) {
		...
		// 公開予定日時（タイムゾーン日時）
		$tz_datetime = $this->main->utils()->convert_to_timezone_datetime($array[self::TS_OUTPUT_RESERVE]);
		$conv_array[self::OUTPUT_ENTITY_RESERVE] 		 = $tz_datetime;
		$conv_array[self::OUTPUT_ENTITY_RESERVE_DISP] = $this->main->utils()->format_datetime($tz_datetime, define::DATETIME_FORMAT_DISP);

		// 処理開始日時（GMT日時）
		$conv_array[self::OUTPUT_ENTITY_START_GMT] 	= $array[self::TS_OUTPUT_START];
		// 処理開始日時（タイムゾーン日時）
		$tz_datetime = $this->main->utils()->convert_to_timezone_datetime($array[self::TS_OUTPUT_START]);
		$conv_array[self::OUTPUT_ENTITY_START]		   = $tz_datetime;
		$conv_array[self::OUTPUT_ENTITY_START_DISP] = $this->main->utils()->format_datetime($tz_datetime, define::DATETIME_FORMAT_DISP);

		// 処理終了日時（タイムゾーン日時）
		$tz_datetime = $this->main->utils()->convert_to_timezone_datetime($array[self::TS_OUTPUT_END]);
		$conv_array[self::OUTPUT_ENTITY_END]	     = $tz_datetime;
		$conv_array[self::OUTPUT_ENTITY_END_DISP] = $this->main->utils()->format_datetime($tz_datetime, define::DATETIME_FORMAT_DISP);
		...
		// 状態
		$conv_array[self::OUTPUT_ENTITY_STATUS_DISP] = $this->main->utils()->convert_status($array[self::TS_OUTPUT_STATUS]);
		// 公開種別
		$conv_array[self::OUTPUT_ENTITY_PUBLISH_TYPE] = $this->main->utils()->convert_publish_type($array[self::TS_OUTPUT_PUBLISH_TYPE]);
		// 登録ユーザID
